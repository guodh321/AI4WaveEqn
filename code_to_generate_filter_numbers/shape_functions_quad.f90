...
! ngi=no of surface quadrature points of the faces.
! ndim=no of dimensions - including time possibly, nface=no of faces of each elemenet, nc no of fields to solve for.
! dxele_dim(idim)=length of element in idim direction. This is not the grid spacing. 
! The grid spacing is dxele_dim/real(nacross-1)
...
!        dxele_dim(:)=1.0*real(nacross-1) ! dimensions 2 is the default. 
        dxele_dim(:)=1.0*real(nacross-1) ! node spacing of 5.0. 
!        dxele_dim(:)=1.0*real(nacross-1) ! dimensions 2 is the default. 
...
! 
        ...
        real, intent( out ) :: vec_a_sfc_all_un(ndim,ndim,max_ncola_sfc_all_un), &
                               vec_b_sfc(ndim,max_nonods_sfc_all_grids), &
                               ml_sfc(max_nonods_sfc_all_grids) 
        real, intent( in ) :: vec_a(ndim,ndim,ncola), vec_b(ndim,nonods), ml(nonods)
        integer, intent( out ) :: nonods_sfc_all_grids, fin_sfc_nonods(max_nlevel+1), nlevel
...
! 
        ...
        integer, intent( in ) :: sfc_nonods_course, sfc_nonods_fine
        integer, intent( in ) :: ndim
        real, intent( out ) :: vec_ml_sfc_course(ndim,sfc_nonods_course)
        real, intent( in ) :: vec_ml_sfc_fine(ndim,sfc_nonods_fine)
! local variables...
        integer i_short, i_long
! 
